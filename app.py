    ...
    if "results" not in st.session_state or "buffer_insights" not in st.session_state.results:
        ...
        with st.spinner("Анализируем отчёты..."):
            try:
                ...
                Структура твоего отчёта должна быть именно такой: Обязательно при обязательно пиши с новой строчки! Рассуждай!
                ...
                /n •	Название SKU — маржинальность за вчера (значение)%
                ...
                /n •	Название SKU — ДРР за вчера (значение)%, и за месяц (значение)%.
                ...
                /n Закончить простым предложением: «Эти категории приносят наибольшую прибыль.»

                /n В конце немного оцени что все нормально в целом только посмотрите расходы на рекламу. 
                /n Добавляй комментарии специалиста в конце каждого блока. В конце
                ...
                Нужно чтобы твой отчет был понятным. Поэтому каждый Товар в новой строчке, красиво, понятно. 
                     {full_report}"""
                )
