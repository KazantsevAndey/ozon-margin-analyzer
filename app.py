        ...
        with st.spinner("Анализируем отчёты..."):
            try:
                ...
                prompt = (
                    ...
                    "- Обязательно указывай цифры когда говоришь о таких категориях как маржа и прибыль." 
                    "- В файле 'buffer_account' сравни общую маржу за вчера и с начала месяца. Если за вчера маржа выше чем за весь месяц то значит она в целом растет. Сделай вывод: маржа растёт, падает или стабильна. \n"
                    "- В таблицах 'low_margin_yesterday' и 'low_margin_month' найди товары с маржой ниже 20%. Напиши: 'Обратите внимание на следующие товары — маржинальность крайне низкая, по ним почти нет заработка.' Обязательно используй такие формулировки\n"

                    "- В таблицах 'high_drr_yesterday' и 'high_drr_month' - товары с у которых всю прибыль сьела реклама. Напиши: 'В этих товарах маркетинговые расходы съедают всю прибыль. Рекомендуется проверить настройки рекламных кампаний.' Используй такую формулиовку и перечисли товары\n"
