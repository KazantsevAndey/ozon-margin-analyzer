    ...
    if "results" not in st.session_state or "buffer_insights" not in st.session_state.results:
        ...
        with st.spinner("Анализируем отчёты..."):
            try:
                ...
                В конце обязательно напиши одним понятным предложением: «Проверьте настройки рекламных кампаний этих товаров, реклама съедает большую часть прибыли.» Порассуждай чуть чуть как это делают специалисты.
