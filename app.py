    ...
    if "results" not in st.session_state or "buffer_insights" not in st.session_state.results:
        ...
        with st.spinner("Анализируем отчёты..."):
            try:
                ...
                Отдельно напиши краткий список таких товаров. Формат:
                ...
                Отдельно топ-3 категории за вчера и топ-3 категории за месяц. Формат:
                ...
                Нужно чтобы твой отчет был понятным. Поэтому если каждый Товар в новой строчке, красиво, понятно. Добавляй комментарии специалиста в конце каждого блока. 
                     {full_report}"""
                )
